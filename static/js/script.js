document.addEventListener('DOMContentLoaded', function() {
    ...
    // عناصر معالجة المجلد
    const folderProcessingArea = document.getElementById('folder-processing-area');
    const folderProgressBar = document.getElementById('folder-progress-bar');
    const folderProgressStatus = document.getElementById('folder-progress-status');
    const processedFolderImages = document.getElementById('processed-folder-images');
    ...
    rotateLeftBtn.addEventListener('click', () => { editorState.rotation -= 90; applyEditorTransforms(); });
    rotateRightBtn.addEventListener('click', () => { editorState.rotation += 90; applyEditorTransforms(); });
    flipHorizontalBtn.addEventListener('click', () => { editorState.flipX = !editorState.flipX; applyEditorTransforms(); });
    flipVerticalBtn.addEventListener('click', () => { editorState.flipY = !editorState.flipY; applyEditorTransforms(); });
    zoomInBtn.addEventListener('click', () => { editorState.scale = Math.min(3, editorState.scale + 0.1); applyEditorTransforms(); });
    zoomOutBtn.addEventListener('click', () => { editorState.scale = Math.max(0.1, editorState.scale - 0.1); applyEditorTransforms(); });
    ...
    function handleDragOver(e) { e.preventDefault(); dropArea.classList.add('dragover'); }
    function handleDragLeave(e) { e.preventDefault(); dropArea.classList.remove('dragover'); }
    ...
    async function processFile(file, isFromFolder = false) {
        ...
        try {
            ...
            const imageResponse = await fetch(`/processed-image/${processedImageFilename}`);
    ...
    async function processNextFileInFolder() {
        if (currentFolderIndex >= folderFiles.length) {
            folderProgressStatus.textContent = 'اكتملت المعالجة! سيتم إعادة تعيين الواجهة بعد 5 ثوانٍ.';
    ...
    function applyEditorTransforms() {
        ...
        const { rotation, scale, flipX, flipY, brightness, contrast, positionX, positionY } = editorState;
        const scaleX = flipX ? -1 : 1;
        const scaleY = flipY ? -1 : 1;
        resultImage.style.transform = `translate(${positionX}px, ${positionY}px) rotate(${rotation}deg) scale(${scale * scaleX}, ${scale * scaleY})`;
        resultImage.style.filter = `brightness(${brightness}%) contrast(${contrast}%)`;
    }

    function resetEditorTransforms() {
        editorState = { ...editorState, rotation: 0, scale: 1, flipX: false, flipY: false, brightness: 100, contrast: 100, positionX: 0, positionY: 0 };
